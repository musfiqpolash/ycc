...
@section('content')
    <form action="" method="POST" enctype="multipart/form-data" id="category_add_form">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">Add Sub Cetegory of {{$category->name}}</div>
            <div class="panel-body">
                <div class="form-group {{$errors->has('name')?'has-error':''}}">
                    <label for="sub_category_name" class="control-label">Sub Category Name</label>
                    <div class="input-group">
                        <input type="text" name="name" id="sub_category_name" class="form-control" value="{{old('name')}}">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">Sub Categories of {{$category->name}}</div>
        <div class="table-responsive">
            <table class="table table-bordered" style="background: white;">
                ...
                <tbody>
                    ...
                    @endforeach
                    @if ($category->subCategory->count('id')==0)
                        <tr>
                            <td colspan="3">Sub Category Table Is Empty</td>
                        </tr>
                    @endif
                </tbody>
            </table>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="cat_name" class="control-label">Sub Category Name</label>
...
@section('customJs')
    <script>
        ...
        $('#cat_edit_btn').on('click',function(){
            $.ajax({
                url:"{{url('admin/sub_category/update')}}",
