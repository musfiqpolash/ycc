...
<style type="text/css">
    ...
    .aacc
    {
        border: 2px solid #0797d4 !important;
    }
    .condition-opt
    {
            ...
            <div class="row">
                ...
                <div class="col-md-12 productDetails">
                    ...
                    <form id="addToCartForm" action="{{url('/addToCart')}}">
                        ...
                        <div class="form-group">
                            <label for="condition"><span class="f-w-light">Condition:</span>Grade A</label><br>
                            <div style="">
                                 <label class="btn condition-opt active-condition" style="margin-top: 0px;">
                                            <input type="radio" name="grade" value="A"
                                                   autocomplete="off" style="display:none;">Grade <span style="font-size: 25px; font-weight: 700;">A</span>
                                </label>
                                <label class="btn condition-opt" style="margin-top: 0px;">
                                            <input type="radio" name="grade" value="B"
                                                   autocomplete="off" style="display:none;">Grade <span style="font-size: 25px; font-weight: 700;">B</span>
                                </label>
                                <label class="btn condition-opt" style="margin-top: 0px;">
                                            <input type="radio" name="grade" value="C"
                                                   autocomplete="off" style="display:none;">Grade <span style="font-size: 25px; font-weight: 700;">C</span>
                                </label>
                            </div>
                        ...
                        {{-- <div class="form-group">
                            ...
                            @foreach($getPrdctClrWise as $clrVal)
                                <a class="activeImg" onclick="changeImage({{$clrVal[0]}})">
                        ...
                        <div class="col-md-12" id="colwi">
                            ...
                            <script type="text/javascript">
                                ...
                                function set_image()
                                {
                                    var colwi=document.getElementById('colwi');
                                    // console.log(colwi.offsetWidth/54);
                                    var phn_num=parseInt(colwi.offsetWidth/54);
                                    document.cookie = "phn_num ="+phn_num;
//set  $phn_num for color toggole 
                                }
                            </script>
                            <div class="row">
                                <label for="color"><span class="f-w-light">Color:</span> <span
                                        id="productClr">{{$product_details[0]->color}}</span></label><br>
                                    @php
                                        $getPrdctClrWise=$product_details->groupBy('color');
                                    @endphp
                                    @if(count($getPrdctClrWise)<=$phn_num)
                                        <div class="form-group">
                                            @foreach($getPrdctClrWise as $clrVal)
                                                <a class="activeImg" onclick="changeImage({{$clrVal[0]}})">
                                                    <img src="{{url('public/uploads/assets/frontend/images/products')}}/{{$clrVal[0]->hasImage[0]->color_img}}"
                                    ...
                                    @if(count($getPrdctClrWise)>$phn_num)
                                        <div class="collapseA">
                                            <a class="col-d collapseDiv collapsed" role="button" data-toggle="collapse"
                                               href="#color" aria-expanded="false">
                                                @php
                                                // $getPrdctClrWise=$product_details->groupBy('color');
                                                $i=0;
                                                @endphp
                                        ...
                                        <div class="collapse text-justify" id="color">
                                            @php
                                            // $getPrdctClrWise=$product_details->groupBy('color');
                                            $j=0;
                                            @endphp
                        ...
                        <div class="form-group" id="productLabelDiv"
                            ...
                            <label for="size"><span class="f-w-light">Memory:</span>16GB</label><br>
                            <div id="productLabelDivA">
                                    @foreach($product_size as $row=> $val)
                                        <label class="btn condition-opt active-condition" style="margin-top: 0px;">
                                            <input type="radio" name="size" value=""
                                                   onclick="cngId('{{$val->id}}','{{$val->price}}','{{$val->size}}','{{$val->name}}','{{$val->color}}')"
                                                   autocomplete="off" style="display: none;"><b style="font-size:17px;">{{$val->size}}</b>
                                        </label>
                                    @endforeach
                                </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label for="wholesale"><span class="f-w-light">Wholesale Price (Piece):</span>100+</label><br>
                            <div style="">
                        ...
                        <div class="form-group">
                            <label for="quantity"><span class="f-w-light">Quantity:</span></label>
                            <input min="1" onblur="chk(this.id)" onchange="chk(this.id)" onkeyup="chk(this.id)" class="form-control" type="number" name="quantity" id="quantity" value="1" style="font-weight: 700; font-size:17px;">
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                        ...
                        <input type="submit" value="Add to Cart" class="form-control btn btn-lg btn-info" style="margin-top: 8px;">
...
@section('customJs')
    ...
    <script>
        ...
        function changeImage(prdct) {
            ...
            $.each(prdct['has_image'], function (key, value) {
            ...
            $('#productLabelDivA').children().remove();

            $.get('{{url('/getSize')}}' + '/' + prdct['name'] + '/' + prdct['color']).done(function (res) {
                var d = $.parseJSON(res);
                $.each(d, function (k, v) {
                    var id = '"' + v['id'] + '"';
                    var price = '"' + v['price'] + '"';
                    ...
                    var color = '"' + v['color'] + '"';
                    console.log(v['size']);
                    $('#productLabelDivA').append(
                        "<label class='btn btn-info' style='margin-right: 3px;'>" +
                        "<input type='radio' name='size' onclick='cngId(" + id + "," + price + "," + size + "," + name + "," + color + ")' value=''>" + v["size"] +
                        "</label>"
                    );
                    $('#productLabelDivA label input').first().prop('checked',true);
                });
            });
        }

        // $('.col-d').click(function(event) {
        //     /* Act on the event */
        //     if($('.col-d').hasClass('collapsed'))
        //     {
        //         $('.col-d').insertAfter('See More');
        //     }
        //     else
        //     {
        //         $('.col-d').insertAfter('See Less');
        //     }
        // });
        ...
        $(document).ready(function () {
            ...
            $('.activeImg img').first().addClass('aacc');

            $('.activeImg').on('click',function () {
                $('.activeImg img').removeClass('aacc');

                $(this).children().addClass('aacc');
            });
        ...
        function cngId(id, price, size, name, color) {
            $('#productPrice').text('$' + price);
            $('#pro_id').val(id);
            $('#productName').text(name + ' - ' + color + ' " ' + size + ' "');
        }
    </script>
@endsection
@section('customCss')
